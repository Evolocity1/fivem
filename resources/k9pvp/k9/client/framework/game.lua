function MP.GetVehicleModel(hash)
    for i = 1, #GetAllVehicleModels() do
        if GetEntityModel(hash) == GetHashKey(GetAllVehicleModels()[i]) then
            return GetAllVehicleModels()[i]
        end
    end
    return ""
end

function MP.SpawnVehicle(vehicle, coords, heading, cb, networked)
...
function MP.Teleport(entity, coords, cb)
...
function MP.GetObjects() -- Leave the function for compatibility
    return GetGamePool('CObject')
end

function MP.GetPeds(onlyOtherPeds)
...
function MP.GetVehicles() -- Leave the function for compatibility
    return GetGamePool('CVehicle')
end

function MP.GetPlayers(onlyOtherPlayers, returnKeyValue, returnPeds)
...
function MP.GetClosestObject(coords, modelFilter)
    return MP.GetClosestEntity(MP.GetObjects(), false, coords, modelFilter)
end

function MP.GetClosestPed(coords, modelFilter)
    return MP.GetClosestEntity(MP.GetPeds(true), false, coords, modelFilter)
end

function MP.GetClosestPlayer(coords)
    return MP.GetClosestEntity(MP.GetPlayers(true, true), true, coords, nil)
end

function MP.GetClosestVehicle(coords, modelFilter)
    return MP.GetClosestEntity(MP.GetVehicles(), false, coords, modelFilter)
end
...
function MP.GetPlayersInArea(coords, maxDistance)
    return EnumerateEntitiesWithinDistance(MP.GetPlayers(true, true), true, coords, maxDistance)
end

function MP.GetVehiclesInArea(coords, maxDistance)
    return EnumerateEntitiesWithinDistance(MP.GetVehicles(), false, coords, maxDistance)
end

function MP.IsSpawnPointClear(coords, maxDistance)
    return #MP.GetVehiclesInArea(coords, maxDistance) == 0
end

function MP.GetClosestEntity(entities, isPlayerEntities, coords, modelFilter)
...
function MP.SetVehicleProperties(vehicle, props)
...
function MP.GetVehicleProperties(vehicle)
    if not DoesEntityExist(vehicle) then
        return
    end
    ...
    return {
        ...
        plate = MP.Trim(GetVehicleNumberPlateText(vehicle)),
        plateIndex = GetVehicleNumberPlateTextIndex(vehicle),

        bodyHealth = MP.Round(GetVehicleBodyHealth(vehicle), 1),
        engineHealth = MP.Round(GetVehicleEngineHealth(vehicle), 1),
        tankHealth = MP.Round(GetVehiclePetrolTankHealth(vehicle), 1),

        fuelLevel = MP.Round(GetVehicleFuelLevel(vehicle), 1),
        dirtLevel = MP.Round(GetVehicleDirtLevel(vehicle), 1),
        color1 = colorPrimary,
...
function MP.RequestModel(modelHash, cb)
...
function MP.RequestStreamedTextureDict(textureDict, cb)
...
function MP.RequestNamedPtfxAsset(assetName, cb)
...
function MP.RequestAnimSet(animSet, cb)
...
function MP.RequestAnimDict(animDict, cb)
...
function MP.RequestWeaponAsset(weaponHash, cb)
