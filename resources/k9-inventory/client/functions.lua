...
function OpenInventory(inventoryType, page)
    if disableInventory and not checkrpf then return end
    TriggerServerEvent("gfx-inventory:server:OpenInventory", inventoryType, nil, nil, page)
end

function UseSlot(id)
    local v = HotbarData[id]
    if not isDead and not isOpened and v and HasItem("inventory", v.name) and not checkrpf then
