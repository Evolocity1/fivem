...
function AddItem(source, inventoryType, itemName, itemCount, info, forceAdd, zrz, inf)
    local infoTypes = {
        ["weapon"] = {ammo=0}
    }
    ...
    local itemData, index, itemlist = GetItemByName(source, inventoryType, itemName)
    if itemData then
...
function RemoveItem(source, inventoryType, itemName, itemCount, zrz)
    local identifier = GetPlayerIdentifiers(source)[1]
    if not identifier then return false end
    local removedInfo = {}
        ...
        if PlayerItems[identifier][inventoryType][index].count == 0 then
            ...
            if inventoryType == "inventory" then
                ...
                if Items[itemName].type == "weapon" then
                    RemoveWeaponFromPed(GetPlayerPed(source), GetHashKey(itemName))
...
function ClearInventory(source, inventoryType)
    ...
    TriggerClientEvent("gfx-inventory:UpdateInventory", source, {}, "inventory")
    TriggerClientEvent("gfx-inventory:SetHotbar", source, Hotbars[identifier])
